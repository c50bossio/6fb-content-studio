...
export default function CarouselStudio({ brandProfile, onNavigateToBrand, onCarouselCreated, hasClaudeKey }: Props) {
  ...
  // Playbook linkage (extract mode only)
  const [activeRunPlaybook, setActiveRunPlaybook] = useState<RunPlaybookMeta | null>(null);
  ...
  const handleExtract = async () => {
    ...
    // Clear previous carousel playbook linkage
    setCarouselPlaybookPostId(undefined);
    setCarouselPlaybookTopicId(undefined);
    setCarouselLinkedTitle(undefined);
    try {
      ...
      const result = await window.electronAPI.extractCarousel({
        ...
        ...(activeRunPlaybook
          ? {
              playbookBrief: {
                topicTitle: activeRunPlaybook.topicTitle,
                pillar: activeRunPlaybook.pillar,
                hookIdea: activeRunPlaybook.hookIdea ?? '',
                visualSuggestion: activeRunPlaybook.visualSuggestion ?? '',
                shotList: activeRunPlaybook.shotList ?? [],
              },
              playbookPostId: activeRunPlaybook.playbookPostId,
              playbookTopicId: activeRunPlaybook.playbookTopicId,
            }
          : {}),
      });
      ...
      if (result.success && result.slides) {
        ...
        setCurrentDeckTitle(activeRunPlaybook?.topicTitle || selectedRun.sourceVideo || 'Untitled');
        setExtractingFrames(false);
        // Persist playbook linkage on the carousel
        if (activeRunPlaybook) {
          setCarouselPlaybookPostId(activeRunPlaybook.playbookPostId);
          setCarouselPlaybookTopicId(activeRunPlaybook.playbookTopicId);
          setCarouselLinkedTitle(activeRunPlaybook.topicTitle);
        } else {
          const genResult = result as { playbookPostId?: string; playbookTopicId?: string };
          if (genResult.playbookPostId) {
            setCarouselPlaybookPostId(genResult.playbookPostId);
            setCarouselPlaybookTopicId(genResult.playbookTopicId);
          }
        }
        onCarouselCreated?.();
      } else { setError((result as any).error || 'Extraction failed'); }
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="shrink-0 border-b border-[#1a1a1a] px-4 py-2 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                ...
                {/* Playbook linkage chip — extract mode only, locked */}
                {mode === 'extract' && carouselLinkedTitle && (
                  <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-green-500/10 border border-green-500/30 shrink-0">
                    ...
                    <span className="text-[10px] text-green-500 font-semibold truncate max-w-[180px]">
                      Linked to Today's Play: {carouselLinkedTitle}
                    </span>
                  </div>
                )}
              </div>
