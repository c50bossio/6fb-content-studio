...
export default function BlogWriter({ brandProfile, onBlogCreated, hasClaudeKey }: Props) {
  ...
  return (
    <div className="h-full flex flex-col bg-[#0a0a0a]">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[240px] border-r border-[#1a1a1a] flex flex-col bg-[#0d0d0d]">  
          <div className="p-4 border-b border-[#1a1a1a]">
            ...
            >
              <option value="">Select a clip run...</option>
              {libraryRuns.map(run => (
                <option key={run.runId} value={run.runId}>
                  {run.sourceVideo || 'Unknown video'} · {run.clips.length} clip{run.clips.length !== 1 ? 's' : ''}
                </option>
              ))}
            </select>
            {!hasClaudeKey ? (
          ...
          {!loading && sections.length === 0 && !error && (
            <div className="flex flex-col items-center justify-center h-full text-center px-8">
              ...
              <p className="text-sm text-[#555] max-w-md">
                Select a recent clip extractor run from the left panel — it will automatically pull the transcript from your IX pipeline output. Then hit <span className="text-[#00C851] font-semibold">Generate Blog Post</span> to create a fully structured, SEO-ready article.
              </p>
              {libraryRuns.length === 0 && (
                <p className="text-xs text-[#333] mt-4 max-w-sm">No clip runs found yet. Extract some clips in the Clip Extractor first.</p>
              )}
            </div>
          )}
      ...
      {scheduleOpen && sections.length > 0 && (
        <ScheduleModal
          isOpen={scheduleOpen}
