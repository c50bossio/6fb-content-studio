...
export function runClipExtractor(
  ...
  return new Promise((resolve) => {
    ...
    if (options.logoPath) {
      args.push('--logo-override', options.logoPath);
    } else {
      args.push('--no-logo');
    }
    ...
    proc.on('close', (code) => {
      ...
      try {
        ...
        if ((code === 0 || code === null) && existsSync(validatedPath)) {
          ...
          if (Notification.isSupported() && formattedClips.length > 0) {
            const firstTitle = formattedClips[0]?.label || formattedClips[0]?.title;
            const notif = new Notification({
              title: '🎬 Your clips are ready!',
              body: firstTitle
                ? `"${firstTitle}" — ${formattedClips.length} clip${formattedClips.length !== 1 ? 's' : ''} extracted`
                : `${formattedClips.length} clip${formattedClips.length !== 1 ? 's' : ''} extracted successfully`,
              silent: false,
            });
            notif.on('click', () => {
              if (mainWindow) {
