...
def parse_srt(file_path: str) -> Transcript:
    ...
    for i, block in enumerate(raw_blocks):
        ...
        # Word is everything from line 3 onward
        word_text = " ".join(lines[2:]).strip()
        if not word_text:
            continue

        pieces = word_text.split()
        if not pieces:
            continue

        piece_dur = (end_sec - start_sec) / len(pieces)
        
        for p_idx, piece in enumerate(pieces):
            w_start = start_sec + (p_idx * piece_dur)
            w_end = w_start + piece_dur
