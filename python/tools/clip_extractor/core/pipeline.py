...
def extract_clip(
    ...
    source_ext = Path(source_video).suffix.lower()
    if source_ext in (".mp4", ".m4v"):
        codec_args = ["-c", "copy"]
    else:
        # Re-encode to h264 for QuickTime/ProRes/HEVC sources
        codec_args = ["-c:v", "libx264", "-preset", "fast", "-crf", "18", "-c:a", "aac"]
...
def analyze(
    ...
    # Split and auto formats use 9x16 for the analysis phase;
    # auto gets resolved to 9x16 or split during the reframe step after multi-face detection
    calc_format = "9x16" if output_format in ("split", "auto") else output_format
    ...
    last_known_y = 0.5
    frames_since_any_signal = 0
    stale_decay_threshold = int(info.fps * 1.0)  # 1.0s of frames

    start_time = time.time()

    # Phase 5: Split-screen mode needs multi-face detection
    is_split_mode = output_format in ("split", "auto")
    ...
    for frame_idx, frame in read_frames(video_path, sample_rate=sample_rate):
        ...
        if bbox is not None:
            ...
            confidence = 0.0
            face_detected = False
            frames_since_any_signal += 1  # incremented here; reset below if any signal detected
        ...
        if pose_detector or saliency_detector:
            ...
            # Keep last_known current from any detected signal, not just face.
            # Prevents stale fallback when face is lost but pose/saliency track.
            if not face_detected and (pose_detected or saliency_detected):
                last_known_x, last_known_y = fused_x, fused_y
        else:
            fused_x, fused_y = face_x, face_y

        # Reset staleness counter if any signal is alive
        if face_detected or pose_detected or saliency_detected:
            frames_since_any_signal = 0
...
def reframe(
    ...
    # Step 2.5: Auto-format detection — decide between solo 9x16 and multi-person split
    if output_format == "auto":
        multi_face_frames = sum(
            1 for kf in crop_path.keyframes
            if kf.all_faces and len(json.loads(kf.all_faces)) >= 2
        )
        total_kf = len(crop_path.keyframes)
        multi_face_pct = (multi_face_frames / total_kf * 100) if total_kf > 0 else 0

        if multi_face_pct >= 15:
            print(f"\n[clip-extractor] 🤖 Auto-Detect: {multi_face_pct:.1f}% multi-face frames "
                  f"→ switching to SPLIT layout for active-speaker framing")
            output_format = "split"
            # Re-save crop_path with corrected format tag
            crop_path.output_format = "split"
            save_crop_path(crop_path, crop_path_file)
            # Also update the suffix so the output file name is correct
            suffix = "split"
        else:
            print(f"\n[clip-extractor] 🤖 Auto-Detect: {multi_face_pct:.1f}% multi-face frames "
                  f"→ solo speaker, using 9x16 reframe")
            output_format = "9x16"
            suffix = "9x16"
